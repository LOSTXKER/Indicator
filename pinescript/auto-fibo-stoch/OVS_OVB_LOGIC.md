...
## 5. Solution — Bounded OVS Movement
...
- OVS marker ขยับได้เฉพาะตอน **เจอ peak ใหม่** → garantizes ว่า OVS time **< new peak time**
...
### 6.2 UP Fibo อาจ stale หลัง OVS ขยับ

UP Fibo สร้างตอน K>80 (ใน `SM_WAIT_OVS → SM_WAIT_OVB` transition) ใช้ `_sLo, _sLoT` snapshot

- ถ้า OVS ขยับใน `SM_WAIT_OVB` ภายหลัง → Fibo anchor ยังอยู่ที่เดิม (stale)
- DN Fibo ของรอบถัดไปใช้ `prevOvsLo` ที่อัปเดตแล้ว → ถูกต้อง

**TODO ถ้าจะแก้:** Recreate UP Fibo เมื่อ OVS ขยับ (หรือใช้ shared reference)

### 6.3 H2 ที่ K กระโดด >80→<20 ในบาร์เดียว
